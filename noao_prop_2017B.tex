...
We ask for 31.5 nights of WIYN+WHIRC time to obtain 58 host galaxy template images
...
\numnights{11}
\lunardays{bright}
\optimaldates{Sep-Jan}
\acceptabledates{Sep-Jan}
\end{obsrun}
...
\unusabledates{
Run 1 is a list of priority objects that we need to observe to make quantitative estimations of when a supernova needs a host galaxy template.
Run 2 is a full list of unobserved host galaxies (Not including Run 1, Run 3, or previously observed hosts).
Run 3 contains host galaxies that are only observable in January.
...
To be able to remove any possible host galaxy flux and give proper errors on our measured fluxes, we would like to observe host galaxy templates for all of our SweetSpot supernovae.
We currently need 58 templates which accounts for $51\%$ of our total sample. 

Additionally, 4 of these supernovae are heavily contaminated by their host but we had not previously prioritized observing a
...
\caption{Postage stamps from 3 fields in Run 1 showing a heavily contaminated field with one lightcurve point (left), supernova on the edge of a galaxy (middle), and a high redshift supernova with faint host (right).}
\end{figure}

\begin{figure}
%\epsscale{0.5}
\plotone{postage_stamps_run2.eps}
\caption{Postage stamps from 3 fields in Run 2 showing three fields not obviously contaminated, but worth getting templates for.}
\end{figure}

\begin{figure}
%\epsscale{0.1}
\plotone{postage_stamps_run3.eps}
\caption{Postage stamps from 3 fields in Run 3 showing a heavily contaminated field with one lightcurve point (left), 91T-like object that is heavily contaminated (middle), and a high redshift supernova removed from its host (right).}
\end{figure}
...
We request a total of 31.5 nights for WHIRC observations of host galaxies of SNeIa and standard stars.  
...
Run 1 consists of host galaxy templates that will help us to quantitatively estimate when a host galaxy needs a template and when it does not.
...
We need to obtain 30 host galaxy references. 
Observations need to be at least 3 times as long as the longest exposure of the field when the supernova was live (assuming 1\arcsec\ seeing).
Our template observations need have better signal to noise than our supernova observations for accurate template subtractions. 
See table below for total exposure times ($\#$ of exp. = total exposure time since each image is 60 seconds).
...
This sample needs 120.5 hours on sky to run the scripts. 
We then add in 90 minutes per night to observe standard stars and to set up on different targets. 
Assuming on average we have 9.5 hours in a night, we need 15 nights on the telescope. 
We then add a 10$\%$ contingency for issues setting up or any unforeseeable problems to give 16.5 nights. 
...
\end{configuration}
\targetsra{00 to 23}			% RA range in hours
\targetsdec{-36 to +69}			% Dec. range in degrees
\specialrequest    % remove and not use for HET runs

\begin{targettable}{Run 2 - All SweetSpot fields not yet observed}
\objid{001}           % specify a 3-digit number for each target
\object{SN2014dk}          % 20 characters maximum
\ra{00:28:11.96}              % e.g., xx:xx:xx.x
\dec{+07:09:43.9}             % e.g., +-xx:xx:xx.x
\epoch{2000}           % e.g., 1950.3
\magnitude{}
\filter{J,H}
\exptime{60}         % in seconds PER EXPOSURE
\nexposures{125,50}      % Number of exposures
\moondays{14}        % Days from new moon, use a number 0-14
\skycond{phot}         % "spec" or "phot"
\seeing{1}          % max allowable PSF FWHM (arcsecs)
\obscomment{}      % 20 characters maximum - REQUIRED COMMAND
...
\objid{002}           % specify a 3-digit number for each target
\object{iPTF13dkx}          % 20 characters maximum
\ra{01:20:53.115}              % e.g., xx:xx:xx.x
\dec{+03:20:23.65}             % e.g., +-xx:xx:xx.x
\epoch{2000}           % e.g., 1950.3
\magnitude{}
\filter{J,H}
\exptime{60}         % in seconds PER EXPOSURE
\nexposures{100,75}      % Number of exposures
\moondays{14}        % Days from new moon, use a number 0-14
\skycond{phot}         % "spec" or "phot"
\seeing{1}          % max allowable PSF FWHM (arcsecs)
\obscomment{}      % 20 characters maximum - REQUIRED COMMAND
...
\objid{003}           % specify a 3-digit number for each target
\object{PS1-13dkh}          % 20 characters maximum
\ra{03:11:16.09}              % e.g., xx:xx:xx.x
\dec{+15:42:58.3}             % e.g., +-xx:xx:xx.x
\epoch{2000}           % e.g., 1950.3
\magnitude{}
\filter{J,H}
\exptime{60}         % in seconds PER EXPOSURE
...
\objid{004}           % specify a 3-digit number for each target
\object{SN2012fr}          % 20 characters maximum
\ra{03:33:36.277}              % e.g., xx:xx:xx.x
\dec{-36:07:35.15}             % e.g., +-xx:xx:xx.x
\epoch{2000}           % e.g., 1950.3
\magnitude{}
\filter{H}
\exptime{60}         % in seconds PER EXPOSURE
\nexposures{09}      % Number of exposures
\moondays{14}        % Days from new moon, use a number 0-14
\skycond{phot}         % "spec" or "phot"
\seeing{1}          % max allowable PSF FWHM (arcsecs)
\obscomment{}      % 20 characters maximum - REQUIRED COMMAND
...
\objid{005}           % specify a 3-digit number for each target
\object{CSS130218}          % 20 characters maximum
\ra{09:23:54.52}              % e.g., xx:xx:xx.x
\dec{+38:58:36.8}             % e.g., +-xx:xx:xx.x
\epoch{2000}           % e.g., 1950.3
\magnitude{}
\filter{H}
\exptime{60}         % in seconds PER EXPOSURE
...
\objid{006}           % specify a 3-digit number for each target
\object{SN2014J}          % 20 characters maximum
\ra{09:55:42.14}              % e.g., xx:xx:xx.x
\dec{+69:40:26.0}             % e.g., +-xx:xx:xx.x
\epoch{2000}           % e.g., 1950.3
\magnitude{}
\filter{J,H,Ks}
\exptime{60}         % in seconds PER EXPOSURE
\nexposures{300,300,200}      % Number of exposures
\moondays{14}        % Days from new moon, use a number 0-14
\skycond{phot}         % "spec" or "phot"
\seeing{1}          % max allowable PSF FWHM (arcsecs)
\obscomment{}      % 20 characters maximum - REQUIRED COMMAND
...
\objid{007}           % specify a 3-digit number for each target
\object{CSS140425}          % 20 characters maximum
\ra{16:10:23.61}              % e.g., xx:xx:xx.x
\dec{+47:04:40.3}             % e.g., +-xx:xx:xx.x
\epoch{2000}           % e.g., 1950.3
\magnitude{}
\filter{J,H}
\exptime{60}         % in seconds PER EXPOSURE
\nexposures{150,75}      % Number of exposures
\moondays{14}        % Days from new moon, use a number 0-14
\skycond{phot}         % "spec" or "phot"
\seeing{1}          % max allowable PSF FWHM (arcsecs)
\obscomment{}      % 20 characters maximum - REQUIRED COMMAND
...
\objid{008}           % specify a 3-digit number for each target
\object{PTF11moy}          % 20 characters maximum
\ra{17:06:18.06}              % e.g., xx:xx:xx.x
\dec{+38:32:19.9}             % e.g., +-xx:xx:xx.x
\epoch{2000}           % e.g., 1950.3
\magnitude{}
\filter{J,H}
\exptime{60}         % in seconds PER EXPOSURE
\nexposures{50,50}      % Number of exposures
\moondays{14}        % Days from new moon, use a number 0-14
\skycond{phot}         % "spec" or "phot"
\seeing{1}          % max allowable PSF FWHM (arcsecs)
\obscomment{}      % 20 characters maximum - REQUIRED COMMAND
...
\objid{009}           % specify a 3-digit number for each target
\object{iPTF13dkl}          % 20 characters maximum
\ra{23:44:57.999}              % e.g., xx:xx:xx.x
\dec{+03:23:40.07}             % e.g., +-xx:xx:xx.x
\epoch{2000}           % e.g., 1950.3
\magnitude{}
\filter{J,H}
\exptime{60}         % in seconds PER EXPOSURE
...
\objid{010}           % specify a 3-digit number for each target
\object{LSQ13cmt}          % 20 characters maximum
\ra{01:02:41.95}              % e.g., xx:xx:xx.x
\dec{-21:52:23.4}             % e.g., +-xx:xx:xx.x
\epoch{2000}           % e.g., 1950.3
\magnitude{}
\filter{J,H}
\exptime{60}         % in seconds PER EXPOSURE
\nexposures{50,75}      % Number of exposures
\moondays{14}        % Days from new moon, use a number 0-14
\skycond{phot}         % "spec" or "phot"
\seeing{1}          % max allowable PSF FWHM (arcsecs)
\obscomment{}      % 20 characters maximum - REQUIRED COMMAND
...
\objid{011}           % specify a 3-digit number for each target
\object{CSS121009}          % 20 characters maximum
\ra{01:11:01.091}              % e.g., xx:xx:xx.x
\dec{-17:28:42.28}             % e.g., +-xx:xx:xx.x
\epoch{2000}           % e.g., 1950.3
\magnitude{}
\filter{J,H}
\exptime{60}         % in seconds PER EXPOSURE
\nexposures{100,150}      % Number of exposures
\moondays{14}        % Days from new moon, use a number 0-14
\skycond{phot}         % "spec" or "phot"
\seeing{1}          % max allowable PSF FWHM (arcsecs)
\obscomment{}      % 20 characters maximum - REQUIRED COMMAND
...
\objid{012}           % specify a 3-digit number for each target
\object{SN2012fk}          % 20 characters maximum
\ra{02:30:52.081}              % e.g., xx:xx:xx.x
\dec{+22:28:46.02}             % e.g., +-xx:xx:xx.x
\epoch{2000}           % e.g., 1950.3
\magnitude{}
\filter{J,H}
\exptime{60}         % in seconds PER EXPOSURE
...
\objid{013}           % specify a 3-digit number for each target
\object{PSNJ030347}          % 20 characters maximum
\ra{03:03:47.9}              % e.g., xx:xx:xx.x
\dec{+00:24:14.6}             % e.g., +-xx:xx:xx.x
\epoch{2000}           % e.g., 1950.3
\magnitude{}
\filter{J,H}
\exptime{60}         % in seconds PER EXPOSURE
...
\objid{014}           % specify a 3-digit number for each target
\object{LSQ12fmx}          % 20 characters maximum
\ra{03:12:52.144}              % e.g., xx:xx:xx.x
\dec{-00:12:12.55}             % e.g., +-xx:xx:xx.x
\epoch{2000}           % e.g., 1950.3
\magnitude{}
\filter{J,H}
\exptime{60}         % in seconds PER EXPOSURE
...
\objid{015}           % specify a 3-digit number for each target
\object{SN2011ha}          % 20 characters maximum
\ra{03:57:40.87}              % e.g., xx:xx:xx.x
\dec{+10:09:55.2}             % e.g., +-xx:xx:xx.x
\epoch{2000}           % e.g., 1950.3
\magnitude{}
\filter{J,H}
\exptime{60}         % in seconds PER EXPOSURE
...
\objid{016}           % specify a 3-digit number for each target
\object{SN2013ar}          % 20 characters maximum
\ra{08:37:44.978}              % e.g., xx:xx:xx.x
\dec{+49:28:32.88}             % e.g., +-xx:xx:xx.x
\epoch{2000}           % e.g., 1950.3
\magnitude{}
\filter{J,H}
\exptime{60}         % in seconds PER EXPOSURE
...
\objid{017}           % specify a 3-digit number for each target
\object{CSS121114}          % 20 characters maximum
\ra{09:02:02.420}              % e.g., xx:xx:xx.x
\dec{+10:18:00.31}             % e.g., +-xx:xx:xx.x
\epoch{2000}           % e.g., 1950.3
\magnitude{}
\filter{J,H}
\exptime{60}         % in seconds PER EXPOSURE
...
\object{PTF11qmo}          % 20 characters maximum
\ra{10:06:49.748}              % e.g., xx:xx:xx.x
\dec{-07:41:12.39}             % e.g., +-xx:xx:xx.x
\epoch{2000}           % e.g., 1950.3
\magnitude{}
\filter{J,H}
\exptime{60}         % in seconds PER EXPOSURE
...
\objid{020}           % specify a 3-digit number for each target
\object{SN2012bh}          % 20 characters maximum
\ra{12:13:37.325}              % e.g., xx:xx:xx.x
\dec{+46:29:00.34}             % e.g., +-xx:xx:xx.x
\epoch{2000}           % e.g., 1950.3
\magnitude{}
\filter{J,H}
\exptime{60}         % in seconds PER EXPOSURE
...
\objid{021}           % specify a 3-digit number for each target
\object{SN2011fe}          % 20 characters maximum
\ra{14:03:05.80}              % e.g., xx:xx:xx.x
\dec{+54:16:25.3}             % e.g., +-xx:xx:xx.x
\epoch{2000}           % e.g., 1950.3
\magnitude{}
\filter{J,H,Ks}
\exptime{60}         % in seconds PER EXPOSURE
...
\objid{022}           % specify a 3-digit number for each target
\object{SN2013bs}          % 20 characters maximum
\ra{17:17:22.03}              % e.g., xx:xx:xx.x
\dec{+41:04:00.2}             % e.g., +-xx:xx:xx.x
\epoch{2000}           % e.g., 1950.3
\magnitude{}
\filter{J,H}
\exptime{60}         % in seconds PER EXPOSURE
...
\objid{023}           % specify a 3-digit number for each target
\object{CSS140501}          % 20 characters maximum
\ra{17:04:13.69}              % e.g., xx:xx:xx.x
\dec{+17:48:39.4}             % e.g., +-xx:xx:xx.x
\epoch{2000}           % e.g., 1950.3
\magnitude{}
\filter{J,H}
\exptime{60}         % in seconds PER EXPOSURE
\nexposures{150,200}      % Number of exposures
\moondays{14}        % Days from new moon, use a number 0-14
\skycond{phot}         % "spec" or "phot"
\seeing{1}          % max allowable PSF FWHM (arcsecs)
\obscomment{}      % 20 characters maximum - REQUIRED COMMAND

\objid{024}           % specify a 3-digit number for each target
\object{ASASSN-14in}          % 20 characters maximum
\ra{18:24:29.61}              % e.g., xx:xx:xx.x
\dec{+52:26:32.83}             % e.g., +-xx:xx:xx.x
\epoch{2000}           % e.g., 1950.3
\magnitude{}
\filter{J,H}
\exptime{60}         % in seconds PER EXPOSURE
...
\objid{025}           % specify a 3-digit number for each target
\object{SN2015G}          % 20 characters maximum
\ra{20:37:25.57}              % e.g., xx:xx:xx.x
\dec{+66:07:11.68}             % e.g., +-xx:xx:xx.x
\epoch{2000}           % e.g., 1950.3
\magnitude{}
\filter{J,H,Ks}
\exptime{60}         % in seconds PER EXPOSURE
\nexposures{25,41,50}      % Number of exposures
\moondays{14}        % Days from new moon, use a number 0-14
\skycond{phot}         % "spec" or "phot"
\seeing{1}          % max allowable PSF FWHM (arcsecs)
\obscomment{Ibn}      % 20 characters maximum - REQUIRED COMMAND

\objid{026}           % specify a 3-digit number for each target
\object{SN2011gf}          % 20 characters maximum
\ra{21:12:24.302}              % e.g., xx:xx:xx.x
\dec{-07:48:52.04}             % e.g., +-xx:xx:xx.x
\epoch{2000}           % e.g., 1950.3
\magnitude{}
\filter{J,H}
\exptime{60}         % in seconds PER EXPOSURE
...
\objid{027}           % specify a 3-digit number for each target
\object{SN2011fs}          % 20 characters maximum
\ra{22:17:19.509}              % e.g., xx:xx:xx.x
\dec{+35:34:49.94}             % e.g., +-xx:xx:xx.x
\epoch{2000}           % e.g., 1950.3
\magnitude{}
\filter{J,H,Ks}
\exptime{60}         % in seconds PER EXPOSURE
\nexposures{75,75,25}      % Number of exposures
\moondays{14}        % Days from new moon, use a number 0-14
\skycond{phot}         % "spec" or "phot"
\seeing{1}          % max allowable PSF FWHM (arcsecs)
\obscomment{}      % 20 characters maximum - REQUIRED COMMAND

\objid{028}           % specify a 3-digit number for each target
\object{SN2012em}          % 20 characters maximum
\ra{22:44:01.66}              % e.g., xx:xx:xx.x
\dec{+15:51:49.3}             % e.g., +-xx:xx:xx.x
\epoch{2000}           % e.g., 1950.3
\magnitude{}
\filter{J,H}
\exptime{60}         % in seconds PER EXPOSURE
\nexposures{100,125}      % Number of exposures
\moondays{14}        % Days from new moon, use a number 0-14
\skycond{phot}         % "spec" or "phot"
\seeing{1}          % max allowable PSF FWHM (arcsecs)
\obscomment{}      % 20 characters maximum - REQUIRED COMMAND

\objid{029}           % specify a 3-digit number for each target
\object{LSQ12fhs}          % 20 characters maximum
\ra{22:52:23.423}              % e.g., xx:xx:xx.x
\dec{-20:36:53.35}             % e.g., +-xx:xx:xx.x
\epoch{2000}           % e.g., 1950.3
\magnitude{}
\filter{J,H}
\exptime{60}         % in seconds PER EXPOSURE
\nexposures{75,125}      % Number of exposures
\moondays{14}        % Days from new moon, use a number 0-14
\skycond{phot}         % "spec" or "phot"
\seeing{1}          % max allowable PSF FWHM (arcsecs)
\obscomment{}      % 20 characters maximum - REQUIRED COMMAND

\objid{030}           % specify a 3-digit number for each target
\object{SN2011io}          % 20 characters maximum
\ra{23:02:47.617}              % e.g., xx:xx:xx.x
\dec{+08:48:08.12}             % e.g., +-xx:xx:xx.x
\epoch{2000}           % e.g., 1950.3
\magnitude{}
\filter{J,H}
\exptime{60}         % in seconds PER EXPOSURE
\nexposures{5-,50}      % Number of exposures
\moondays{14}        % Days from new moon, use a number 0-14
\skycond{phot}         % "spec" or "phot"
\seeing{1}          % max allowable PSF FWHM (arcsecs)
\obscomment{}      % 20 characters maximum - REQUIRED COMMAND
...
\adc{}
\end{configuration}
\targetsra{12 to 16}			% RA range in hours
\targetsdec{-17 to +61}			% Dec. range in degrees
\specialrequest    % remove and not use for HET runs
\begin{targettable}{Run 3 - Targets only available in January}
\objid{001}           % specify a 3-digit number for each target
\object{SN2011jh}          % 20 characters maximum
\ra{12:47:14.42}              % e.g., xx:xx:xx.x
\dec{-10:03:47.2}             % e.g., +-xx:xx:xx.x
\epoch{2000}           % e.g., 1950.3
\magnitude{}
\filter{J,H}
\exptime{60}         % in seconds PER EXPOSURE
...
\objid{002}           % specify a 3-digit number for each target
\object{SN2011iy}          % 20 characters maximum
\ra{13:08:58.38}              % e.g., xx:xx:xx.x
\dec{-15:31:04.0}             % e.g., +-xx:xx:xx.x
\epoch{2000}           % e.g., 1950.3
\magnitude{}
\filter{J,H}
\exptime{60}         % in seconds PER EXPOSURE
...
\objid{003}           % specify a 3-digit number for each target
\object{PTF11qri}          % 20 characters maximum
\ra{12:47:06.276}              % e.g., xx:xx:xx.x
\dec{-06:19:49.46}             % e.g., +-xx:xx:xx.x
\epoch{2000}           % e.g., 1950.3
\magnitude{}
\filter{J,H}
\exptime{60}         % in seconds PER EXPOSURE
...
\objid{005}           % specify a 3-digit number for each target
\object{SN2012bo}          % 20 characters maximum
\ra{12:50:45.215}              % e.g., xx:xx:xx.x
\dec{-14:16:07.69}             % e.g., +-xx:xx:xx.x
\epoch{2000}           % e.g., 1950.3
\magnitude{}
\filter{J,H}
\exptime{60}         % in seconds PER EXPOSURE
...
\objid{006}           % specify a 3-digit number for each target
\object{SN2013cs}          % 20 characters maximum
\ra{13:15:14.839}              % e.g., xx:xx:xx.x
\dec{-17:57:55.24}             % e.g., +-xx:xx:xx.x
\epoch{2000}           % e.g., 1950.3
\magnitude{}
\filter{J,H,Ks}
\exptime{60}         % in seconds PER EXPOSURE
...
\objid{007}           % specify a 3-digit number for each target
\object{SN2013bt}          % 20 characters maximum
\ra{14:21:15.185}              % e.g., xx:xx:xx.x
\dec{+61:34:15.42}             % e.g., +-xx:xx:xx.x
\epoch{2000}           % e.g., 1950.3
\magnitude{}
\filter{J,H}
\exptime{60}         % in seconds PER EXPOSURE
\nexposures{75,100}      % Number of exposures
\moondays{14}        % Days from new moon, use a number 0-14
\skycond{phot}         % "spec" or "phot"
\seeing{1}          % max allowable PSF FWHM (arcsecs)
\obscomment{}      % 20 characters maximum - REQUIRED COMMAND
...
\objid{008}           % specify a 3-digit number for each target
\object{PS1-14ra}          % 20 characters maximum
\ra{14:41:28.44}              % e.g., xx:xx:xx.x
\dec{+09:25:58.7}             % e.g., +-xx:xx:xx.x
\epoch{2000}           % e.g., 1950.3
\magnitude{}
\filter{J,H}
\exptime{60}         % in seconds PER EXPOSURE
\nexposures{75,50}      % Number of exposures
\moondays{14}        % Days from new moon, use a number 0-14
\skycond{phot}         % "spec" or "phot"
\seeing{1}          % max allowable PSF FWHM (arcsecs)
\obscomment{}      % 20 characters maximum - REQUIRED COMMAND
...
\objid{009}           % specify a 3-digit number for each target
\object{PSNJ1502499}          % 20 characters maximum
\ra{15:02:49.96}              % e.g., xx:xx:xx.x
\dec{+48:47:06.2}             % e.g., +-xx:xx:xx.x
\epoch{2000}           % e.g., 1950.3
\magnitude{}
\filter{J,H,Ks}
\exptime{60}         % in seconds PER EXPOSURE
\nexposures{75,50,75}      % Number of exposures
\moondays{14}        % Days from new moon, use a number 0-14
\skycond{phot}         % "spec" or "phot"
\seeing{1}          % max allowable PSF FWHM (arcsecs)
\obscomment{}      % 20 characters maximum - REQUIRED COMMAND
...
\objid{011}           % specify a 3-digit number for each target
\object{SN2012bp}          % 20 characters maximum
\ra{16:18:12.451}              % e.g., xx:xx:xx.x
\dec{+36:28:51.87}             % e.g., +-xx:xx:xx.x
\epoch{2000}           % e.g., 1950.3
\magnitude{}
\filter{J,H}
\exptime{60}         % in seconds PER EXPOSURE
\nexposures{25,50}      % Number of exposures
\moondays{14}        % Days from new moon, use a number 0-14
\skycond{phot}         % "spec" or "phot"
\seeing{1}          % max allowable PSF FWHM (arcsecs)
\obscomment{}      % 20 characters maximum - REQUIRED COMMAND
