...
\unusabledates{
Run 1 is a list of priority objects that we need to observe to make quantitative estimations on when a supernova needs a host galaxy template.
Run 2 is a full list of unobserved host galaxies (Not including Run 1, Run 3, or previously observed hosts).
Run 3 contains host galaxies whose supernovae were observed in a Spring semester and are only observable in January.
We can make use of T$\&$E nights throughout the semester.
}
...
%Observing the same fields for these XX (how many observed since March 2015?) supernovae and the standard SweetSpot calibration star fields with the dual-band GPS system running for another semester will provide important information to tie down our WIYN+WHIRC magnitudes of the stars in the SN fields we use for calibration.  
%We plan to observe more Persson standard stars that span a wider range of colors than the select handful we used for consistency across the nights during the regular survey.
...
%We will use this additional time with the system to provide to all KPNO users the water absorption spectral response function based on the observations of the GPS PWV system throughout the time it has been on the mountain.
...
\object{PSNJ08351668}          % 20 characters maximum
...
\object{PSNJ10292799}          % 20 characters maximum
...
\objid{009}           % specify a 3-digit number for each target
\object{PSNJ15024996}          % 20 characters maximum
